(ns marketadmin.operation
  "OperationActor -- one market-administration operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (MarketOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Market Administration Governor
  ...
  One graph run = one market-administration operation (intake ->
  ...
  decision to a human operator (the licensed exchange administrator).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:listing/admit`/`:trade-halt/lift` ALWAYS reach this
  node when the governor is clean -- see `marketadmin.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [marketadmin.marketopsllm :as marketopsllm]
...
(defn build
  ...
  `marketadmin.store/Store`).
  opts:
    :advisor      -- a `marketadmin.marketopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (marketopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MarketOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (marketopsllm/-advise advisor store request)]
            {:proposal p :audit [(marketopsllm/trace request p)]})))

      ;; Market Administration Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
