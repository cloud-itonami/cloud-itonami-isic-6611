(ns marketadmin.portable-cljs-test-runner
  ...
  `marketadmin.store-contract-test`, which exercises the langchain.db
  ...
  -m if combined, hence -Sdeps for the extra path):
    ...
    clojure -Sdeps '{:paths [\"src\" \"test\"]}' \\
      ...
      --output-to target/tests.cjs -c marketadmin.portable-cljs-test-runner
  ...
  (:require [clojure.test :as t :refer [run-tests]]
            [marketadmin.facts-test]
            [marketadmin.governor-contract-test]
            [marketadmin.kernels.gate-test]
            [marketadmin.phase-test]
            [marketadmin.registry-test]
            [marketadmin.store-contract-test]))
...
(defn -main []
  (run-tests 'marketadmin.facts-test
             'marketadmin.registry-test
             'marketadmin.phase-test
             'marketadmin.kernels.gate-test
             'marketadmin.governor-contract-test
